#!/usr/bin/env python3
"""Detect source-main drift and project its promoted dependency blast radius."""
...
from federation_surface import load_effective_claims, repository_registry  # noqa: E402
...
def github_headers() -> dict[str, str]:
    headers = {
        ...
        "User-Agent": "FPDG-source-drift-watch/0.3",
...
def map_paths_to_claims(repo_id, paths, claims):
...
def aggregate_impact(graph, changed_claims):
    best = {}
...
def render_markdown(report):
    lines = ["# FPDG Source Drift Report", "", f"Status: **{report['status']}**", ""]
    for source in report["sources"]:
        lines.append(
            f"- **{source['repository_id']}** — {source['status']} — "
            f"locked `{source['locked_repository_head'][:12]}` / "
            f"current `{source['current_main'][:12]}`"
        )
        lines.append(
            f"  - represented scientific source: "
            f"`{source['represented_source_commit'][:12]}`"
        )
    ...
    lines.extend([
        "",
        f"Promoted downstream claims provisionally requiring revalidation: **{len(report['impacted'])}**",
        "",
    ])
    ...
    lines.extend([
        "",
        "`CANDIDATE_ONLY` edges were excluded. Semantic export diagnosis follows this provisional detector.",
    ])
    return "\n".join(lines) + "\n"
...
def main() -> int:
    ...
    try:
        ...
        if set(sources) != set(registry):
            raise RuntimeError(
                f"lock/registry mismatch: lock={sorted(sources)} registry={sorted(registry)}"
            )
        claims = load_claim_rows()
        graph = load_graph()
        source_reports = []
        all_changed_claims = set()
        ...
        drifted = [
            row["repository_id"] for row in source_reports if row["status"] == "DRIFT"
        ]
        report = {
