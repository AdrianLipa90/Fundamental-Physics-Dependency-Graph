#!/usr/bin/env python3
"""Generate all four pinned source exports and reconcile them against the canonical DAG."""
...
def main() -> int:
    ...
    if set(sources) != set(registry):
        print(
            f"FAIL: source_heads repositories mismatch: sources={sorted(sources)} registry={sorted(registry)}",
            file=sys.stderr,
        )
        return 1
    ...
    except (OSError, ValueError, ExportError, yaml.YAMLError, json.JSONDecodeError) as exc:
        print(f"FAIL: {exc}", file=sys.stderr)
        return 1

    receipt = {
        "schema": "FPDG_SOURCE_EXPORT_ROUNDTRIP_V0_1",
    ...
    if all_problems:
        return 2
    print("PASS: four-source dependency export roundtrip reconciles with canonical DAG")
    return 0
